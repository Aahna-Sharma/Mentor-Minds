...
(function () {
  const UPLOADED_FALLBACK =
    "/mnt/data/04e98284-8520-4447-8743-9882b6946b53.png"; // last-resort fallback
  ...
  function applyUserToDOM({ name, email, photoURL }) {
    ...
    if (userAvatar && avatarSrc) {
      try {
        ...
        userAvatar.setAttribute("data-avatar-set", "1");
      } catch (e) {}
    }
  ...
  async function resolveUserInfo(timeout = 1200) {
    ...
    const possibleName =
      ...
      "";
    const possibleEmail =
      (qs("menuEmail") && qs("menuEmail").textContent) || "";
    if (possibleName || possibleEmail)
      return { name: possibleName, email: possibleEmail, photoURL: "" };
    return null;
  }
  ...
  function observeAndReapply(desired) {
    const nodesToWatch = [];
    const uA = qs("userAvatar");
    if (uA) nodesToWatch.push(uA);
    const mA =
      (qs("userMenu") && qs("userMenu").querySelector(".user-menu-avatar")) ||
      null;
    if (mA) nodesToWatch.push(mA);
    const mName = qs("menuName");
    if (mName) nodesToWatch.push(mName);
    const mEmail = qs("menuEmail");
    if (mEmail) nodesToWatch.push(mEmail);
  ...
  function initDropdown() {
    ...
    userToggle.addEventListener("pointerdown", toggleFromPointer, {
      passive: false,
    });
  ...
  async function init() {
    ...
    try {
      ...
      console.warn("[hdr] auth listener warn", e);
    }
